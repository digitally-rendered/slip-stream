...
# ---------------------------------------------------------------------------
# AuditTrail — MongoDB persistence (mongomock-motor)
# ---------------------------------------------------------------------------


class TestAuditTrailMongoDB:

    @pytest.mark.asyncio
    async def test_record_to_mongodb(self, mock_db):
        """record() with a real (mocked) db inserts into the collection."""
        audit = AuditTrail(collection_name="audit_log", in_memory=False)

        entry = await audit.record(
            operation="create",
            schema_name="widget",
            entity_id="ent-mongo-1",
            user_id="user-1",
            changes={"name": "Mongo Widget"},
            db=mock_db,
        )

        assert entry.operation == "create"
        # Verify document was actually inserted
        doc = await mock_db["audit_log"].find_one({"entity_id": "ent-mongo-1"})
        assert doc is not None
        assert doc["operation"] == "create"
        assert doc["schema_name"] == "widget"
        assert doc["user_id"] == "user-1"

    @pytest.mark.asyncio
    async def test_get_history_from_mongodb(self, mock_db):
        """get_history() queries the MongoDB collection with the correct filter."""
        audit = AuditTrail(collection_name="audit_log", in_memory=False)

        await audit.record(
            operation="create",
            schema_name="widget",
            entity_id="ent-mongo-2",
            user_id="user-1",
            db=mock_db,
        )
        await audit.record(
        ...
        await audit.record(
            operation="create",
            schema_name="widget",
            entity_id="ent-mongo-3",
            user_id="user-1",
            db=mock_db,
        )

        history = await audit.get_history("ent-mongo-2", db=mock_db)
        assert len(history) == 2
        ops = {h["operation"] for h in history}
        assert ops == {"create", "update"}

    @pytest.mark.asyncio
    async def test_get_user_activity_from_mongodb(self, mock_db):
        """get_user_activity() returns only entries for the requested user."""
        audit = AuditTrail(collection_name="audit_log", in_memory=False)
        ...
        activity = await audit.get_user_activity("alice", db=mock_db)
        assert len(activity) == 2
        assert all(doc["user_id"] == "alice" for doc in activity)

    @pytest.mark.asyncio
    async def test_record_no_db_no_memory_warns(self, caplog):
        """When in_memory=False and db=None, a warning is logged and entry is lost."""
        import logging

        audit = AuditTrail(in_memory=False)

        with caplog.at_level(logging.WARNING, logger="slip_stream.core.audit"):
            entry = await audit.record(
                operation="create",
                schema_name="widget",
                entity_id="ent-lost",
        ...
        assert entry.operation == "create"
        assert any("audit entry lost" in record.message for record in caplog.records)

    @pytest.mark.asyncio
    async def test_change_delta_computation(self):
        """Verify old_data vs new_data diff is captured in changes for update."""
        audit = AuditTrail(in_memory=True)

        old_data = {"name": "Original Name", "color": "blue"}
        new_data = {"name": "Updated Name", "color": "blue"}

        # Compute a simple delta: only changed fields
        changes = {
        ...
        assert entry.changes["name"]["old"] == "Original Name"
        assert entry.changes["name"]["new"] == "Updated Name"
        # Color did not change — should not appear in delta
        assert "color" not in entry.changes

        stored = audit._entries[0]
        assert stored["changes"]["name"]["new"] == "Updated Name"
